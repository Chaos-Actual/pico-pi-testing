 ...
 */
void getRiseSet(unsigned long unixTime, unsigned long *sunrise, unsigned long *sunset, struct Sun Sun) {
  ...
  // Hour angle
  float omega0 = 360 / (2 * pi) * acos((sin(-0.83 / 360 * 2 * pi) - sin(Sun.lat / 360 * 2 * pi) * sin(delta / 360 * 2 * pi)) / (cos(Sun.lat / 360 * 2 * pi) * cos(delta / 360 * 2 * pi)));
...
unsigned long getRise(unsigned long unixTime) {
  unsigned long rise;
  unsigned long set;
  getRiseSet(unixTime, &rise, &set, Sun);
  return rise;
}
